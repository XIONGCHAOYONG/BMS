...
//TODO 执行日志
    ...
    public void DoNotificationAndLend() {
        ...
        List<Long> oldSoonOverdueLendIds = lendMapper.findOldSoonOverdue(LendStatus.SOON_OVERDUE);
        List<Long> oldOverdueLendIds = lendMapper.findOldOverdue(LendStatus.OVERDUE);
        //在原有的将要逾期信息中筛选出逾期的
        List<Long> newOverdueLendIds = lendMapper.getOverDueIds(oldSoonOverdueLendIds);
        if (!newOverdueLendIds.isEmpty()){
        //修改状态为逾期、重置通知天数为1
        ...
        notificationMapper.resetDays(newOverdueLendIds);
    }
        //获得原来的将要逾期信息
        oldSoonOverdueLendIds.removeAll(newOverdueLendIds);
        List<Long> SoonOverdueLendIds = oldSoonOverdueLendIds;
        //对oldOverdueLendIds和SoonOverdueLendIds的天数进行修改+1和-1
        if(!oldOverdueLendIds.isEmpty())
             notificationMapper.upDays(oldOverdueLendIds);
        if(!SoonOverdueLendIds.isEmpty())
             notificationMapper.downDays(SoonOverdueLendIds);
